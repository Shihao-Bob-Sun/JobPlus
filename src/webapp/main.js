...
(function() {
	...
	function getLocationFromIP() {   // 这个function只是拿到地址信息，还没从后端拿job数据呢
		// get location from http://ipinfo.io/json
		var url = 'http://ipinfo.io/json'         // 这个网站可以拿到我的实时IP地址信息
		var data = null;
	...
	// -----------------------------------
	function register() {
		var username = document.querySelector('#register-username').value;     // 拿到表单里的用户input
		...
		if (username === "" || password == "" || firstName === ""    // 检查是否为空
				|| lastName === "") {
			showRegisterResult('Please fill in all fields');
			return;
		}

		if (username.match(/^[a-z0-9_]+$/) === null) {   // 检查用户名是否符合规则
			showRegisterResult('Invalid username');
			return;
		}
		password = md5(username + md5(password));   // 对密码进行加密
	...
	// Step 8: valid session 持久登陆
	

	
	// 只执行这一个函数，其他函数都在init里被call到
	init();
